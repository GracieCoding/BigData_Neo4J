...
class QueryCollaborator {
  ...
  def queryCollaborator (x: String, y: Float) : Unit = {
    ...
    var myList2: List[((((String, String),String), BigDecimal), String)] = List()
    var myList: List[((((String, String),String), BigDecimal), String)] = List()
    val myInterestActor = actor {
      ...
      myList = stream4Interest.map(row => {row[String]("user") -> row[String]("lname") -> row[String]("interest") -> row[BigDecimal]("total") -> row[String]("result")}).toList
    }
    val mySkillActor = actor {
      ...
      myList2 = stream4Skills.map(row => {row[String]("username") -> row[String]("lname") -> row[String]("skill") -> row[BigDecimal]("result") -> row[String]("orgName")}).toList
    }
    var infoList: List[(BigDecimal, List[((((String, String),String), BigDecimal), String)])] = List()
    var sum: BigDecimal = 0

    while (myInterestActor.getState != Actor.State.Terminated || mySkillActor.getState != Actor.State.Terminated){
